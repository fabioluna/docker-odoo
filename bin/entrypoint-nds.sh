...
if [ -f /opt/.ssh/id_rsa ]; then
  ...
  if [ ! -d nds-third-party ]; then
    git clone --single-branch -b $ODOO_VERSION git@gitlab.com:nds-odoo/nds-third-party.git
  fi
  if [ ! -d odoo-nds ]; then
    git clone --single-branch -b $ODOO_VERSION git@gitlab.com:nds-odoo/odoo-nds.git
  fi
